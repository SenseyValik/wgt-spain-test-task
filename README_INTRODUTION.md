...
# Початок тестування
відкрийте сторінку в браузері http://your-domain.test/docs/api#/
на ній ви можете виконувати тестові запити до API. Тестові дані уже внесені відносно створених першопочаткових даних
```bash
# створення 30000 offers в jobs для імпорту
## краще не вказувати більше 50 000 записів бо на локальному сервері можуть бути малі налаштування памяті і процес буде падати
php artisan import:generate 30000
php artisan import:generate 30000
# використовувати інший алгоритм для імпорту по одній позиції
php artisan import:generate 30000 --row-by-row

# запуск обробки jobs
## збільшив час життя процесу
php artisan queue:work redis --timeout=600 
```
в браузері по АПІ надсилати ID імпорту який цікавить щоб моніторити процес його виконання
...
Рішення
1. Ділимо увесь payload на чанки 500-1000-5000 (можна вказувати) де для кожного чанку буде окрема транзакція
2. Починаємо імпорт чанку
