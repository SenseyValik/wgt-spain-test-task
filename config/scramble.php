...
return [
    ...
    'info' => [
        ...
        'description' => <<<'MARKDOWN'
            Import accommodation offers from suppliers, find the cheapest actual offer per
            property, and reserve one safely.

            **Imports are asynchronous.** `POST /imports` answers `202` and queues the work,
            so a queue worker has to be running (`php artisan queue:work redis`). Poll
            `GET /imports/{import}` for the outcome.

            **Imports are idempotent.** The same `supplier` + `external_import_id` returns the
            existing record and queues nothing. An offer already seen under a different import
            is updated and repointed, never duplicated.

            **Reservations are safe under concurrency.** Booking takes a pessimistic row lock on
            the offer and re-reads availability inside the transaction, so two requests for the
            last unit cannot both succeed — the loser gets `409`. A repeated `client_reference`
            is an idempotent replay: `200` instead of `201`, and no second unit taken.

            Prices are integers in minor units — `72500` is 725.00 EUR.
            MARKDOWN,
    ],
